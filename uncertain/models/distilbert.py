...
class DistilBertLabelSmoothingSequenceClassification(DistilBertPreTrainedModel):
...
class DistilBertVAEForSequenceClassification(DistilBertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        ...
        self.vae = VAE(config, hidden_size=config.dim, latent_size=100, var_scale=0.1, eta_bn_prop=0.0)
    ...
    def classify(self, representation):
        pooled_output = self.pre_classifier(representation)  # (bs, dim)
        pooled_output = nn.ReLU()(pooled_output)  # (bs, dim)
        pooled_output = self.dropout(pooled_output)  # (bs, dim)
        return self.classifier(pooled_output)  # (bs, num_labels)
    ...
    def forward(
        ...
        hidden_state = distilbert_output[0]  # (bs, seq_len, dim)
        cls_representation = hidden_state[:, 0]  # (bs, dim)
...
class DistilBertVAEForSequenceClassificationMaskAnalysis(DistilBertVAEForSequenceClassification):
...
class DistilBertVAEForSequenceAnalysis(DistilBertVAEForSequenceClassification):
    ...
    def forward(
        ...
        influntial_idxs, influntial_idx_coss, word_cosss = self.vae.cos_delta_by_dimension_batches(cls_representation_recon-cls_representation, perturbed, hidden_state)
...
class DistilBertBNNForSequenceClassification(DistilBertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        ...
        self.bnn = BNN(config, hidden_size=config.dim, latent_size=100)
...
class DistilBertBNNForSequenceAnalysis(DistilBertBNNForSequenceClassification):
    ...
    def forward(
        ...
        influntial_idxs, influntial_idx_coss, word_cosss = self.bnn.cos_delta_by_dimension_batches(cls_representation_recon-cls_representation, perturbed, hidden_state)
        recon_labels = torch.argmax(self.sm(logits), dim=1)
        pesudo_labels = torch.argmax(self.sm(self.classify(cls_representation)), dim=1)
...
class DistilBertMCDForSequenceClassification(DistilBertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
